...
execute unless score #cryonova_vis therm.config matches 1.. at @e[type=armor_stand,tag=cryonova_entity,tag=fuse,scores={cryonova=1..,therm_track=20..}] run function thermodynamic:cryonova/fuse_vis
execute unless score #cryonova_sfx therm.config matches 1.. at @e[type=armor_stand,tag=cryonova_entity,tag=fuse,scores={cryonova=1..,therm_track=20..}] run playsound minecraft:block.glass.break master @a[distance=..50]

# 1 second effect timer reset
scoreboard players set @e[type=armor_stand,tag=cryonova_entity,tag=fuse,scores={cryonova=1..,therm_track=20..}] therm_track 0
# fire the bij
