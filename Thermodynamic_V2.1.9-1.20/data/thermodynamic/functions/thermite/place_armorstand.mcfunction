...
#execute if score #thermite_fuse therm.config < #kill_as therm.config run scoreboard players operation #thermite_fuse therm.config = #kill_as therm.config
execute if score #thermite_fuse therm.config matches 61.. run scoreboard players set #thermite_fuse therm.config 60
execute if score #thermite_fuse therm.config matches ..-1 run scoreboard players set #thermite_fuse therm.config 0

summon armor_stand ~0.5 ~ ~0.5 {Tags:["thermite_entity","thermodynamic"],Invulnerable:1,PersistenceRequired:1,NoGravity:1,Invisible:1,Marker:1,Small:1}
...
execute as @e[type=armor_stand,tag=thermite_entity,limit=1,sort=nearest] at @s positioned ~ ~ ~ store result score @s therm_void unless block ~1 ~ ~ minecraft:magma_block unless block ~ ~ ~1 minecraft:magma_block unless block ~-1 ~ ~ minecraft:magma_block unless block ~ ~ ~-1 minecraft:magma_block unless block ~ ~1 ~ minecraft:magma_block unless block ~ ~-1 ~ minecraft:magma_block run kill @s
execute as @e[type=armor_stand,tag=thermite_entity,limit=1,sort=nearest] if score @s therm_void matches 0 run kill @s
