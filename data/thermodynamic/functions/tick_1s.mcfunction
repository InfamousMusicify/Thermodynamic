...
# fuse tag remover
execute as @a[tag=fuse_tweak] run function thermodynamic:trigger/kill_fuse_timer

# original ground craftin. more reliable funny enough
execute at @e[type=minecraft:item,nbt={Item:{id:"minecraft:blue_ice",Count:1b}}] at @e[type=minecraft:item,nbt={Item:{id:"minecraft:netherite_block",Count:1b}},distance=..1] if block ~ ~-0.01 ~ minecraft:enchanting_table run function thermodynamic:craft/therm_start
# old new crppy crafting table crafting, pig spawn egg, left for old players
execute as @a[scores={therm_craft=1..}] run function thermodynamic:craft/therm_start_old
# disenchant
