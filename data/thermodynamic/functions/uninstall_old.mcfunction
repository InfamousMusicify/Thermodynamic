# Uninstall
scoreboard objectives remove co_time
scoreboard objectives remove co_cryobomb_ca
scoreboard objectives remove cr_time
scoreboard objectives remove cryobomb
scoreboard objectives remove bomb_time
scoreboard objectives remove m_cryobomb
#
#advancement revoke @a only cryobomb:cryobomb/trigger/place_cryobomb
#advancement revoke @a only cryobomb:cryobomb/trigger/place_m_cryobomb
#

scoreboard objectives remove co_thermite_ca
scoreboard objectives remove therm_time
#advancement revoke @a only thermite:thermite/trigger/place_thermite


scoreboard objectives remove co_thermite_ca
scoreboard objectives remove therm_time

scoreboard objectives remove co_time
scoreboard objectives remove co_cryobomb_ca
scoreboard objectives remove cr_time
scoreboard objectives remove cryobomb
scoreboard objectives remove bomb_time
scoreboard objectives remove m_cryobomb

scoreboard objectives remove thermo_cycle

scoreboard objectives remove therm-fuse

tag @a remove azide_sfx_off
tag @a remove azide_vis_off

tag @a remove thermite_sfx_off
tag @a remove thermite_vis_off

tag @a remove cryonova_sfx_off
tag @a remove cryonova_vis_off

tag @a remove cryobomb_sfx_off
tag @a remove cryobomb_vis_off

tag @a remove cryocherry_sfx_off
tag @a remove cryocherry_vis_off

tag @a remove cryopop_sfx_off
tag @a remove cryopop_vis_off

tag @a remove thermodynamic_off
tag @a remove therm_fuse_off

# old but here for admins in need
kill @e[type=minecraft:armor_stand,tag=azide_entity]
kill @e[type=minecraft:armor_stand,tag=thermite_entity]

kill @e[type=minecraft:armor_stand,tag=cryonova_entity]
kill @e[type=minecraft:armor_stand,tag=cryobomb_entity]
kill @e[type=minecraft:armor_stand,tag=cryocherry_entity]
kill @e[type=minecraft:armor_stand,tag=cryopop_entity]