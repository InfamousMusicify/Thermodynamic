...
execute unless score #rosite_vis therm.config matches 1.. at @e[type=minecraft:armor_stand,tag=rosite_entity,tag=fuse,scores={rosite=1..,therm_track=20..}] run particle minecraft:lava ~ ~ ~ 1 1 1 1 50 force
execute unless score #rosite_sfx therm.config matches 1.. at @e[type=minecraft:armor_stand,tag=rosite_entity,tag=fuse,scores={rosite=1..,therm_track=20..}] run playsound minecraft:block.redstone_torch.burnout master @a[distance=..10]

scoreboard players set @e[type=minecraft:armor_stand,tag=rosite_entity,tag=fuse,scores={rosite=1..,therm_track=20..}] therm_track 0
# fire the bij
