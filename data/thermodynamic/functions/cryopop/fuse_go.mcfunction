...
execute unless entity @e[tag=cryopop_fused] as @e[type=minecraft:armor_stand,tag=cryopop_entity,tag=fuse,scores={cryopop=1..},limit=1] run schedule function thermodynamic:cryopop/fuse 1t
...
execute unless score #cryopop_sfx therm.config matches 1.. at @s run playsound minecraft:block.glass.break master @a[distance=..10]
